...
async def cmd_my_car(message: Message):
    ...
    try:
        ...
        if car_info:
            await message.reply(f"🚗 <b>Ваш автомобиль:</b>\n{car_info}", parse_mode="HTML")
        else:
...
async def cmd_to(message: Message):
    ...
    try:
        ...
        contact_info = """🔧 <b>Запись на ТО</b>

Для записи на техническое обслуживание обращайтесь:

📞 <b>Телефон:</b> +7 (XXX) XXX-XX-XX
🕒 <b>Время работы:</b> Пн-Пт: 9:00-18:00, Сб: 9:00-15:00
📍 <b>Адрес:</b> [Адрес сервисного центра]

Или оставьте заявку через бота, и мы свяжемся с вами."""
        
        await message.reply(contact_info, parse_mode="HTML")
    except Exception as e:
...
async def cmd_start(message: Message):
    ...
    try:
        ...
        if not user:
            await message.reply("""👋 <b>Добро пожаловать в Car Assistant Bot!</b>

Я помогу вам с вопросами о вашем автомобиле.

❌ <b>У вас пока нет доступа к функциям бота.</b>
...
Используйте /help для получения подробной справки.""", parse_mode="HTML")
        else:
            if user['allowed']:
...
Просто напишите любой вопрос о вашем автомобиле, и я постараюсь помочь!""", parse_mode="HTML")
                else:
                    await message.reply("""👋 <b>Добро пожаловать в Car Assistant Bot!</b>

✅ <b>У вас есть доступ к функциям бота.</b>
...
- Как подготовить автомобиль к зиме?""", parse_mode="HTML")
            else:
                await message.reply("""👋 <b>Добро пожаловать в Car Assistant Bot!</b>
...
async def handle_text_message(message: Message):
    ...
    try:
        ...
        if response:
            ...
            await message.reply(f"{response}", parse_mode="HTML")
        else:
